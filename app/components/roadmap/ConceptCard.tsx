...
export default function ConceptCard({ concept, onClick, completed = false, disabled = false }: ConceptCardProps) {
  ...
  const style = {
    ...
    cursor: completed || disabled ? (disabled ? 'not-allowed' : 'pointer') : 'grab'
  }
      ...
      >
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[10px] font-bold uppercase tracking-wider ${completed ? 'text-emerald-500/70' : 'text-zinc-500'}`}>
        ...
        <h4 className={`text-xs font-bold leading-tight mb-1.5 ${completed ? 'text-zinc-300' : disabled ? 'text-zinc-500' : 'text-white'}`}>
          {concept.title}
        </h4>
