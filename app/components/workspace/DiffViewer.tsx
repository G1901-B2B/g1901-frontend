...
export default function DiffViewer({
  ...
  useEffect(() => {
    ...
    if (scrollRef.current) {
      const firstChange = scrollRef.current.querySelector(
    ...
    if (allLines.filter((l) => l.type !== "header").length === 0) {
      return (
        <div className="text-zinc-500 text-sm p-4">
          No changes detected (file is identical to HEAD)
        </div>
      );
    }
    ...
    return unifiedLines.map((line, index) => {
      if (line.type === "header") {
        ...
        if (match) {
          ...
          if (counts.removed > 0) {
            parts.push(
              `${counts.removed} line${counts.removed > 1 ? "s" : ""} removed`
            );
          }
          if (counts.added > 0) {
            parts.push(
              `${counts.added} line${counts.added > 1 ? "s" : ""} added`
            );
          }
          ...
          return (
            <div
              key={index}
              className="bg-zinc-800/50 text-zinc-400 font-semibold px-4 py-2 border-y border-zinc-700"
            >
              Changes at line {newStart}
              {summary}
            </div>
          );
        }
  ...
  const renderSplitDiff = () => {
    ...
    if (filteredOldLines.length === 0 && filteredNewLines.length === 0) {
      return (
        <div className="text-zinc-500 text-sm p-4">No changes detected</div>
      );
    }
