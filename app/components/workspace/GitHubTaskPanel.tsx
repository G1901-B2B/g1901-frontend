...
export default function GitHubTaskPanel({
  ...
  const parsedDescription = (() => {
    ...
    if (task.task_type === "verify_commit" && desc.includes("pencil icon")) {
      desc =
        "Finalize your first contribution by committing changes to your repository. 1) Ensure your name is listed correctly as the author. 2) You can edit files directly on GitHub using the pencil icon, or work locally and push your changes. After committing, paste the commit URL below.";
    }
    ...
    if (points.length > 0) {
      ...
      const verifyRegex =
        /(.*?\.)\s*(We'll verify.*|Our system.*|Please.*|After creating.*|Once finished.*|After committing.*)/i;
      ...
      if (match) {
        ...
        if (task.task_type === "github_profile") {
          footer =
            "Our system will now perform a quick scan to ensure your profile meets these requirements.";
        } else if (task.task_type === "create_repo") {
          footer =
            "Once your repository is live, paste the URL below so we can verify your project setup.";
        } else if (task.task_type === "verify_commit") {
          footer =
            "After pushing your changes, paste the commit URL or SHA below to verify your contribution.";
        } else if (task.task_type === "github_connect") {
          footer =
            "After providing your PAT and accepting the terms, we'll verify your token and store it securely.";
        } else {
          footer =
            "Please provide the required information below to proceed with verification.";
        }
      }
    }
  ...
  const extractRepo = (url: string) => {
    ...
    const match = url
      .trim()
      .match(/github\.com\/([a-zA-Z0-9-]+)\/([a-zA-Z0-9._-]+)/);
    return match
      ? { owner: match[1], repo: match[2].replace(/\.git$/, "").split("/")[0] }
      : null;
  };
  ...
  const extractCommit = (input: string, projectUrl: string) => {
    ...
    const urlMatch = cleanInput.match(
      /github\.com\/([a-zA-Z0-9-]+)\/([a-zA-Z0-9._-]+)\/commit\/([a-f0-9]{7,40})/
    );
    if (urlMatch)
  ...
  const handleGitHubConnect = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/github/consent`,
        {
          method: "POST",
          headers: {
  ...
  const handleVerify = async () => {
    ...
    try {
      if (task.task_type === "github_profile") {
        ...
        result = {
          ...
          checks: [
            ...
            {
              label: "Bio & Name complete",
              passed: !!data.name && !!data.bio,
              detail: data.bio ? "Found bio" : "Bio missing",
            },
          ],
        };
        ...
        const contents = await githubFetch(
          `https://api.github.com/repos/${repo.owner}/${repo.repo}/contents/`
        );
        ...
        const hasLicense = contents.some(
          (item: { name?: string; type?: string }) =>
            item.type === "file" &&
            ["license", "licence"].includes(item.name?.toLowerCase() || "")
        );
        const unexpectedItems = contents.filter(
          (item: { name?: string; type?: string }) => {
            if (item.type !== "file") return true;
            const name = item.name?.toLowerCase() || "";
            return (
              name !== "readme.md" && name !== "license" && name !== "licence"
            );
          }
        );
        const hasOnlyReadmeAndLicense =
          hasReadme && hasLicense && unexpectedItems.length === 0;
        result = {
          success:
            !data.private &&
            isDifferentFromCurriculum &&
            hasOnlyReadmeAndLicense,
          checks: [
            { label: "Repository found", passed: true, detail: data.full_name },
            {
              ...
              detail: isDifferentFromCurriculum
                ? "Different repository"
                : "Matches curriculum repository",
            },
            {
        ...
        result = {
          ...
          checks: [
            ...
            {
              label: "Author recognized",
              passed: true,
              detail: data.commit?.author?.name || "Verified",
            },
            {
              label: "Commit message found",
              passed: hasMessage,
      ...
      if (result.success) {
        ...
        if (
          task.task_type !== "github_profile" &&
          task.task_type !== "create_repo" &&
          task.task_type !== "verify_commit"
        ) {
          const token = await getToken();
          await completeTask(project.project_id, task.task_id, token);
        }
  ...
  const config = configMap[task.task_type] || {
    icon: Github,
    ...
    <div className="space-y-10 animate-fade-in">
        ...
        {task.hints && task.hints.length > 0 && (
          <div className="grid grid-cols-1 gap-3">
            {task.hints.map((hint, i) => (
              <div
                key={i}
                className="flex gap-3 p-4 rounded-xl bg-zinc-900/50 border border-zinc-800/50 group hover:border-blue-500/20 transition-all"
              >
      ...
      <Card className="bg-zinc-900/30 border-zinc-800 shadow-xl overflow-hidden group">
        <CardContent className="p-0">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-yellow-500/5 border border-yellow-500/20">
                  <div className="flex items-start gap-3">
                    ...
                    <label
                      htmlFor="consent"
                      className="text-[12px] text-zinc-400 leading-relaxed cursor-pointer"
                    >
          ...
          {verificationResult && (
            <div
              className={`px-6 py-4 border-t ${status === "success" ? "bg-emerald-500/5 border-emerald-500/10" : "bg-red-500/5 border-red-500/10"}`}
            >
              {verificationResult.checks.map((check, i) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-[11px] font-bold ${check.passed ? "text-emerald-500/80" : "text-red-500/80"}`}
                    >
                      {check.label}
                    </span>
                  </div>
