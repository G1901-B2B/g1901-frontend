...
export default function TerminalComponent({
  ...
  const initializeTerminal = useCallback(() => {
    log(
      "INIT",
      `Container: ${!!containerRef.current}, Terminal: ${!!Terminal}, Already init: ${!!terminalRef.current}`
    );

    // Safety checks: ensure refs and container exist, and container has dimensions
    if (!containerRef.current || !Terminal || !FitAddon || !WebLinksAddon)
      return;
        ...
        try {
          ...
          if (rect && rect.width > 0 && rect.height > 0) {
            ...
            const terminalElement =
              containerRef.current?.querySelector(".xterm");
            if (terminalElement) {
  ...
  useEffect(() => {
    log(
      "TOKEN_EFFECT",
      `terminalReady: ${terminalReady}, hasToken: ${!!token}`
    );
    ...
    async function fetchToken() {
      ...
      try {
        ...
        if (!newToken || !mounted) {
          log(
            "TOKEN",
            `Token fetch result: token=${!!newToken}, mounted=${mounted}`
          );
          return;
        }
  ...
  useEffect(() => {
    log(
      "CONNECT_EFFECT",
      `token: ${!!token}, terminalReady: ${terminalReady}, initialized: ${initializedRef.current}`
    );
