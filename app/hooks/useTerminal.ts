...
function getWebSocketUrl(workspaceId: string, token: string, sessionId?: string): string {
...
export function useTerminal(options: UseTerminalOptions): UseTerminalReturn {
  ...
  const connect = useCallback(() => {
    ...
    if (!workspaceId || !token) {
      log('CONNECT', `Missing params - workspaceId: ${!!workspaceId}, token: ${!!token}`)
      setStatus('error')
      return
    }
    ...
    ws.onclose = (event) => {
      log('WS_CLOSE', `WebSocket closed - code: ${event.code}, reason: ${event.reason}`)
  ...
  }, [workspaceId, token, sessionId, onOutput, onConnected, onError, onDisconnect, autoReconnect, reconnectDelay, cleanup])
  ...
  const sendInput = useCallback((data: string) => {
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      ...
      log('SEND_INPUT', `Cannot send - WebSocket not open (state: ${wsRef.current?.readyState})`)
    }
  }, [])
