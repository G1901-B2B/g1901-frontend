  ...
  try {
    ...
    if (!res.ok) {
      const error = await res
        .json()
        .catch(() => ({ detail: "Failed to start task session" }));
    ...
    if (err instanceof TypeError && err.message === "Failed to fetch") {
      throw new Error(
        `Cannot connect to API at ${API_BASE}. Make sure the backend server is running.`
      );
    }
    throw err;
  }
...
export async function completeTaskSession(
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/task-sessions/${sessionId}/complete`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!res.ok) {
      const error = await res
        .json()
        .catch(() => ({ detail: "Failed to complete task session" }));
...
export async function getTaskSessionDiff(
  ...
  try {
    ...
    if (!res.ok) {
      const error = await res
        .json()
        .catch(() => ({ detail: "Failed to fetch diff" }));
