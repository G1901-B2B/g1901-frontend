...
const LOG_ENDPOINT = 'http://127.0.0.1:7242/ingest/92926a72-8323-4be7-928d-d365e86b8ef8'
const LOG_PATH = 'c:\\projects\\.cursor\\debug.log'

export function logPerformance(metric: string, data: Record<string, any>) {
  const logEntry = {
...
export function measureAsync<T>(
  ...
  return fn().then(
    ...
    (error) => {
      const duration = performance.now() - start
      logPerformance(name, { duration: Math.round(duration), error: error.message })
      throw error
    }
  )
}
