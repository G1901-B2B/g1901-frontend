...
export default function WorkspaceClient() {
  ...
  const [nextDayFirstConcept, setNextDayFirstConcept] = useState<string | null>(
    null
  );
    ...
    async function fetchTaskDetails() {
      ...
      try {
        ...
        if (
          ...
          const [progress, dayData, conceptData, roadmapData] =
            await Promise.all([
          ...
          const sortedDays = (roadmapData.days || []).sort(
            (a, b) => a.day_number - b.day_number
          );
          const nextDay = sortedDays.find(
            (d) =>
              d.day_number === currentDayNumber + 1 &&
              d.generated_status === "generated"
          );

          if (nextDay) {
            // Fetch next day's details to get first concept
            try {
              const nextDayData = await getDayDetails(
                data.project.project_id,
                nextDay.day_id,
                token
              );
              if (nextDayData.concepts && nextDayData.concepts.length > 0) {
                const firstConcept = nextDayData.concepts.sort(
  ...
  const nextNavigation: NextNavigation | null = (() => {
    ...
    const currentTaskIndex = conceptDetails.tasks.findIndex(
      (t) => t.task_id === taskId
    );
    ...
    if (
      currentTaskIndex >= 0 &&
      currentTaskIndex < conceptDetails.tasks.length - 1
    ) {
      return {
        type: "task",
    ...
    const sortedConcepts = [...dayDetails.concepts].sort(
      (a, b) => a.order_index - b.order_index
    );
    const currentConceptIndex = sortedConcepts.findIndex(
      (c) => c.concept_id === taskDetails.concept.concept_id
    );

    if (
      currentConceptIndex >= 0 &&
      currentConceptIndex < sortedConcepts.length - 1
    ) {
