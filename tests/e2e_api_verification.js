...
async function runE2ETests() {
  ...
  if (!CLERK_TOKEN) {
    console.error(
      "❌ Error: TEST_CLERK_TOKEN is missing. Provide a valid Clerk JWT to run real tests."
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (health.status === 200) {
      ...
      console.log(
        "⚠️ Backend health check failed (expected if /health is not implemented)."
      );
    }

    // 2. List Projects
    console.log("\n--- [2/5] Project Listing ---");
    const projectsResp = await axios.get(`${API_BASE_URL}/projects`, {
      headers: authHeader,
    });
    console.log(
      `✅ Successfully fetched ${projectsResp.data.length} projects.`
    );
    ...
    if (projectsResp.data.length > 0) {
      ...
      const detailResp = await axios.get(
        `${API_BASE_URL}/projects/${firstProject.project_id}`,
        { headers: authHeader }
      );
      console.log(
        `✅ Successfully fetched details for project: ${detailResp.data.project_name}`
      );
    } else {
    ...
    if (projectsResp.data.length > 0) {
      ...
      const roadmapResp = await axios.get(
        `${API_BASE_URL}/projects/${projectId}/roadmap`,
        { headers: authHeader }
      );
      console.log(
        `✅ Successfully fetched roadmap with ${roadmapResp.data.concepts.length} concepts.`
      );
    }
    ...
    if (projectsResp.data.length > 0) {
      ...
      const filesResp = await axios.get(
        `${API_BASE_URL}/workspaces/${workspaceId}/files?path=/workspace`,
        { headers: authHeader }
      );
      console.log(
        `✅ Successfully listed ${filesResp.data.length} files in workspace root.`
      );
    }
